...
1. `bun add @riml-ds/tokens @riml-ds/css`
2. ルート CSS で `@import "@riml-ds/tokens/tokens.css"; @import "@riml-ds/tokens/themes/qrcc.css";`
3. 既存の `--qrcc-*` を **別名**にする：
   ```css
   @layer rd.overrides {
   ...
   ```
   `tools/migrate/alias-map.<brand>.json` に対応表を置き、`bunx @riml-ds/mcp migrate alias --brand qrcc`
   で生成する。
...
`@riml-ds/css` の reset / base に切り替え、アプリ側の reset を消す。`@layer` 順を揃える。

## 段階 3: 共通部品

`rd-button` / `rd-text-field` / `rd-live-region` / `rd-skip-link` / `rd-dialog` を `@riml-ds/react` で
...
`bunx @riml-ds/mcp migrate run --from 1 --to 2` で適用。
