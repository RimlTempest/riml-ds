...
髪の青（`accent.500`）は**文字を載せない**装飾（区切り、メーターの区分、アイコン地）にだけ使う。
窓の帯の「広げる」の丸は、帯のインクの上で 3:1 を取るため 1 段明るい `accent.300` を使う（§7.1）。
...
signature は「riml の印」。選択中の目印、ブランド表示に使う。
...
- **左端の丸は装飾ではなく操作**（2026-09-08 のオーナー判断、ADR-0014）。並びは左から
  **閉じる（×）・広げる（□）・たたむ（−）**。使わない操作の丸は**描かない**（押せない丸を置かない）。
  1 つも使わないなら帯の左は空欄で、タイトルは帯の中央のまま。
- **丸の塗りは操作ごとに riml の 3 色**（2026-09-20 のオーナー判断、ADR-0015）:
  閉じる = `chrome.control.close`（ほっぺの桃 `danger.300`）、広げる = `chrome.control.expand`
  （髪の青 `accent.300`）、たたむ = `chrome.control.collapse`（紙 `neutral.0`）。中の記号は
  `chrome.default`（インク）のまま。丸は帯に対しても記号に対しても **3:1 以上**（全モード・全テーマで機械検査。
  記号の色 = 帯の色なので 1 つの対で両方を見ている）。**色だけで操作を伝えない** — 記号と `aria-label` が常に付く。
- **記号は幾何のまま riml の筆致で描く**（ADR-0015）。× は 8 度傾けて手で書いた印に寄せ、□ は角丸を
  一辺の 1/3 弱（一辺 9.6 に `rx 3`）まで丸め、− は端を丸く短く取る。線は `stroke-width 2.4` /
  `stroke-linecap round` の 1 本書きで、0.75rem に縮めても 1 本の線として読める太さにする。
  参考にした画面の記号を同じ角度・太さ・角丸で写さない（§9）。
- 丸の見た目は 1.25rem、当たり判定は `sizing.target-min`（2.75rem）四方。hover / focus-visible は丸の外側に
  `chrome.text` の 2px リング（塗りは変えない — 3 色のコントラストを壊さない）。
- **丸の並びは利用側が調整できる**: `--rd-window-control-gap`（丸どうしの間隔。既定 `0`）、
  `--rd-window-control-size`（丸の直径。既定 `1.25rem`）、`--rd-window-glyph-size`（記号の大きさ。既定 `0.75rem`）。
  既定の `0` は「詰める」ではなく**当たり判定（2.75rem 四方）が隣り合う**状態で、丸と丸の見た目の隙間は
  すでに 1.5rem ある。**隙間を負にしない**（押せる四角が重なり、どちらを押したか分からなくなる）。
  当たり判定は変数では縮まない（AAA の 44px は部品が守る）。既定値は帯（`:host` / `.rd-window-bar`）に置く——
  丸自身に書くと利用側の指定が届かない。
- 骨格は 2 つの出口を持つ:
...
- 帯の色は変えられる: `data-tone="accent"`（`accent.default` + `text.on-accent`）、`data-tone="warning"` / `"danger"`
  （`status.*.default` + `text.on-status`）。文字色は必ず対応する `on-*`。**丸は tone でも 3 色のまま**
  （操作の目印であって帯の装飾ではない）。
...
- ダイアログ = 窓 + `shadow.overlay`。帯の左端の **×（閉じる）が閉じるボタン**（§7.1 と同じ丸。桃の丸に傾いた ×）。`persistent` のときは
...
- **記号の筆致も写さない。** × / □ / − という幾何は共通でも、角度・太さ・角丸・丸の色は riml のもの（§7.1、ADR-0015）。
  「参考にした画面と同じに見える」は、それだけで直す理由になる。
...
**正は riml-ds 側にある**（2026-09-24 の確認）。テーマはプロダクト（qrcc / noter）の実装を
写したものではなく、riml-ds が承認したブランドのパレット。プロダクトが riml-ds の構造を
決めることはない。
...
- **プロダクト固有の名前・段・例外を riml-ds に足さない。** テーマが持てるのは `color.palette.*` の
  値だけで、`system/tokens/test/invariants.test.ts` が機械的に固定する。プロダクトの中だけで
  要る値は、プロダクトのリポジトリに置く。
- AAA はテーマにも等しくかかる。通らない色は**そのブランドの色を直す**（riml-ds 側で緩めない）。
- テーマの置き場所は規模で決める。今（オーナー 1 人・プロダクト 2 つ）は riml-ds の中に置き、
  AAA を riml-ds の CI で強制する。プロダクトが増えたら、riml-ds は「差し替えてよい palette と
  AAA の制約」だけを公開し、テーマ本体はプロダクト側に移す。
