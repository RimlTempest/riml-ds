...
import { contract } from './command.contract.js'
import { computeStates } from './command.logic.js'
...
export class RdCommand extends LitElement {
  ...
  override firstUpdated(): void {
    ...
    if (result.kind === 'missing') {
      console.error(`[rd-command] 契約の子が必要（不足: ${result.roles.join(', ')}）`)
    }
  ...
  override updated(): void {
    syncStates(
      this.#internals,
      computeStates({ malformed: !this.#contractOk, filtering: false, empty: false }),
    )
  }
