...
BASE="${RIML_DS_BASE_REF:-origin/main}"
...
- [ ] \`.claude/skills/riml-ds-typescript\` / \`riml-ds-tdd\` を読んだ（TS を書く場合）
- [ ] \`.claude/skills/riml-ds-element\` / \`riml-ds-css\` を読んだ（部品・CSS を書く場合）
...
- [ ] 所有ディレクトリ外を編集していない
- [ ] a11y 手動確認（UI 変更がある場合）: \`.claude/skills/../system/guidelines/accessibility.md\`
