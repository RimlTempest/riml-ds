 ...
 * `.claude/skills/riml-ds-typescript/SKILL.md` for the rationale behind each
 * rule, and `tools/lint/README.md` for the ADR each one comes from.
 */

/** oxlint exposes the linted path as `context.filename`; older builds only had `getFilename()`. */
...
const noClass = {
  meta: {
    ...
    docs: {
      description:
        'Disallow classes outside *.element.ts. Model behaviour with functions and pass collaborators as arguments (function DI).',
    },
    messages: {
      noClass:
        'class is not allowed here. Custom Elements (*.element.ts) are the only exception (ADR-0005). Elsewhere use a factory function returning an object of closures, and inject collaborators as parameters (DIP).',
    },
  },
  create(context) {
    // ADR-0005: Custom Elements は class でしか定義できない。*.element.ts だけ許す。
    if (fileOf(context).endsWith('.element.ts')) return {}
