# Plan 005: Storybook 10・addon-vitest / a11y（AAA）・描画後 HTML の markuplint・Playwright VRT・addon-mcp
...
- **Priority**: P0
- **Effort**: L
- **Risk**: MEDIUM（Storybook 10 + Vitest 4 の addon-vitest 連携、Docker 上の VRT、shadow DOM の HTML 直列化）
- **Depends on**: 004（`chore/devops`（010）とは独立。CI への組み込みは 010 が行う）
- **Category**: direction
- **Planned at**: commit `0014780`, 2026-09-07（plan 004 のマージ後に着手する）

## Why this matters

「story = テストケース = ドキュメント = エージェントの参照面」を 1 つにする。ここで
axe（AAA タグ）・markuplint（描画後 DOM）・VRT（ライト/ダーク × 2 幅）が story ごとに回るようになると、
以降の部品追加は story を書くだけで品質ゲートに乗る。addon-mcp は開発中のエージェントに
「いまある story と docs」を配る面（ADR-0010）。

## Current state

- plan 004 完了：`library/elements/src/{button,text-field,live-region,skip-link,dialog}/` に部品があり、
  `*.stories.ts` は**存在しない**。`library/elements/custom-elements.json` と `tools/cem/registry.json` がコミット済み。
...
| -------------------- | ----------------------------------------------------------- | ---------------------------------------------------- |
| Storybook dev        | `bun run storybook`（→ `apps/storybook`、port 6006）        | ブラウザで 5 部品 × 8 story                          |
...
- `library/elements/src/*/*.stories.ts`（5 部品 × 1 ファイル。**story ファイルだけ**。部品本体は触らない）
- `tools/cem/src/argtypes.ts` + `tools/cem/src/core/argtypes.ts`（CEM → Storybook `argTypes`。**`tools/cem/src/wrappers/` は plan 006**）
- `e2e/vrt/**`、`e2e/a11y/**`、`e2e/Dockerfile`、`e2e/package.json`、`e2e/playwright.config.ts`
- root：`.mcp.json`、`package.json`（scripts `storybook` / `storybook:build` / `render` / `lint:html` / `vrt` / `vrt:update` / `a11y`、devDeps）、
...
**Verify**: `bun run gen` → `apps/storybook/.storybook/generated/argtypes.ts` に 5 タグ分。テスト 5 件 pass

### Step 3: 5 部品の story（8 種）
...
const meta = {
  ...
  args: { variant: 'primary', 'rd-press': fn() },
  render: (args) => html`<rd-button variant=${args.variant} ?loading=${args.loading} ?disabled=${args.disabled} @rd-press=${args['rd-press']}>保存</rd-button>`,
} satisfies Meta
export default meta
...
export const Default: Story = {
  play: async ({ canvasElement, args }) => {
    const btn = within(canvasElement).getByRole('button', { name: '保存' })
    await userEvent.click(btn)
    await expect(args['rd-press']).toHaveBeenCalledTimes(1)
  },
}
export const Variants: Story = { render: () => html`…4 variant…` }
export const Disabled: Story = { args: { disabled: true }, play: … /* click しても発火しない、focus できる */ }
...
`satisfies Meta` の `Meta` は web-components 用の型（Context7 で `Meta<Args>` の正確な import を確認。`as` は禁止、`satisfies` は可）。
`within(canvasElement).getByRole` は shadow DOM を**またがない** → `storybook/test` の `within` が shadow を見ないなら
`canvasElement.querySelector('rd-button')?.shadowRoot?.querySelector('button')` を `play` で使う（`?.` を使い、`!` は使わない）。
**shadow をまたぐ query が必要なら `apps/storybook/.storybook/shadow.ts` に `queryShadow(root, selector)` を 1 つ置き、story から alias `@rd-shadow` で import。**

他 4 部品も同型で。`text-field`：`Invalid`（`required` 空で blur → エラー文言）、`play` で `userEvent.type` → `FormData` に値。
`live-region`：`Default` の `play` で `announce()` → shadow 内テキスト。`skip-link`：`Default` の `play` で Tab → 表示、Enter → `#main` に focus
（story 内に `<main id="main" tabindex="-1">` を置く）。`dialog`：`Default` の `play` で `show()` → `dialog.open`、Esc → 閉じる + opener focus。
**すべての story で `parameters.a11y` 除外を書かない**。違反が出たら部品のバグとして報告（STOP ではなく報告して継続。story は残す）。

**Verify**: `bun run storybook:build` exit 0。story 数 = 5 × 8 以上（`jq '[.entries[] | select(.type=="story")] | length' apps/storybook/storybook-static/index.json` ≥ 40）
...
skip-link / dialog はキーボード導線（Tab → Enter → focus 先）。
...
`"a11y": "bash scripts/vrt.sh --project a11y"`。`scripts/` は `chore/scaffold` 所有だが `vrt.sh` は新規ファイル → **`scripts/lanes.tsv` の `feat/storybook` に `scripts/vrt.sh` を足す**
（lanes.tsv も新規行の追記は許容。`guard` が所有権を検査するなら先に追記）。

初回ベースライン：`bun run vrt:update` → `e2e/vrt/__screenshots__/**/*.png`（5 部品 × 8 story × 4 + forced + reduced ≈ 180 枚。**1 枚 ≤ 100 KB を目安**、
...
`scripts/guard.sh` に 2 検査追加：（a）`grep -rl "a11y:.*rules" library/elements/src/**/*.stories.ts` 中の `reason` の数 ≤ `custom-elements.json` の部品数、
...
- `storybook` project：40+ story（addon-vitest。各 story が a11y チェック込みで 1 テスト）
- `e2e/vrt`：story × 4 + forced + reduced（Docker）、`e2e/a11y`：5 Default × axe + 2 導線
- `bun run lint:html`：40+ HTML

## Done criteria

- [ ] `bun run test` exit 0（`storybook` project 含む）、`bun run test -- --project storybook` で 40+ pass、a11y 違反 0
- [ ] `bun run render && bun run lint:html` exit 0
- [ ] `bun run vrt` exit 0、`e2e/vrt/__screenshots__/` がコミット済み、合計 ≤ 15 MB
- [ ] `bun run a11y` exit 0
- [ ] 5 部品すべてに 8 story、`parameters.a11y` 除外 0
